...
输入框在模型选择与发送之间只保留一个麦克风，点击后底栏显示点线波形、取消、停止和发送。浏览器 AudioWorklet 持续采集音频，经认证 API 和 `dsh-voice` 的 `voice_stream` 送到本地 Zipformer，识别文字持续进入草稿；SenseVoice 在句尾修正文字、数字与标点。无需 API Key 或云端识别，但首次使用需要浏览器麦克风权限（HTTPS 或 localhost）。录音最长两分钟；取消恢复原有草稿，停止保留最终文字，发送先补齐音频再提交。切换对话和断开设备会停止录音；手动编辑优先，不覆盖用户改动。语音模型与 Python 服务由外层项目配置，后端改动需要重启 DSH Profile。
